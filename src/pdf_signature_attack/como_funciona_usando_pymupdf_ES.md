...
```bash
sudo apt install ghostscript  # Para flattening avanzado
```
...
## 7. Uso en Conferencias

* **Demostración paso a paso**: cada función puede ejecutarse individualmente para mostrar efectos sobre la firma.
* **Visualización de ataques**: comparar archivos en `outputs/`: `signed.pdf` vs `attacked_rewrite.pdf` y `attacked_incremental_pymupdf.pdf`.
...
* **Consistencia**: Estructura similar al script de PikePDF para facilitar comparaciones.
...
* Muestra técnicas de **verificación, detección y mitigación** de ataques.
* La nueva estructura con carpeta `outputs/` mejora la organización y facilita su uso en entornos académicos o administrativos.